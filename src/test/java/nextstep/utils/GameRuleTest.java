...
class GameRuleTest {
    ...
    void numberLength_입력한_숫자_길이_확인(String data) {
        assertAll(
                () -> assertNotNull(data),
                () -> assertTrue(data.length() == MAX_LENGTH, "입력한 숫자 길이 오류")
        );
    }
    ...
    void numberPattern_입력한_문자가_숫자인지_여부(String data) {
    ...
    void duplicateNumber_숫자_중복_여부1(String data) {
        HashSet<String> hashSet = new HashSet<>(Arrays.asList(data.split(":")));
        String[] dataArray = hashSet.toArray(new String[0]);

        assertTrue(String.join("", dataArray).length() == MAX_LENGTH, "중복된 숫자 존재");
    }

    @ParameterizedTest
    @DisplayName("숫자 중복 여부2")
    @CsvSource({"1:2:3,1", "1:3:5,2"})
    void duplicateNumber_숫자_중복_여부2(String data, String number) {
    ...
    void userInputNumberValid_사용자가_입력한_숫자_유효성_검사_숫자_길이와_범위(String number) {
        assertAll(
                () -> assertNotNull(number),
                () -> assertTrue(number.length() == MAX_LENGTH, "입력한 숫자 길이 오류"),
    ...
    void userInputNumberValid_사용자가_입력한_숫자_유효성_검사_중복된_숫자_검사(String number) {
        assertTrue(String.join("", new HashSet<>(Arrays.asList(number.split(""))).toArray(new String[0])).length() == MAX_LENGTH, "중복된 숫자 존재");
    }

    @ParameterizedTest
    @DisplayName("컴퓨터가 입력한 숫자 유효성 검사")
    @CsvSource({"1:2:3,1", "1:3:5,2"})
    void computerInputNumberValid_컴퓨터가_입력한_숫자_유효성_검사(String data, String number) {
        String[] dataArray = data.split(":");
    ...
    void generateHint(String computerNumber, String userNumber) {
        int strikeCount = 0;
        int ballCount = 0;
    ...
    void strikeCount(String computerNumber, String userNumber, int currentIndex) {
        String[] computerNumbers = computerNumber.split(":");
        String[] userNumbers = userNumber.split(":");
        computerNumber = String.join("", computerNumbers);
        assertTrue(computerNumber.indexOf(userNumbers[currentIndex]) == currentIndex, "스트라이크 건수 0");
    }

    @ParameterizedTest
    @DisplayName("볼 건수")
    @CsvSource({"1:2:3,1:4:5,1", "1:2:3,5:2:1,2"})
    void ballCount(String computerNumber, String userNumber, int currentIndex) {
        String[] computerNumbers = computerNumber.split(":");
        String[] userNumbers = userNumber.split(":");
        computerNumber = String.join("", computerNumbers);
    ...
    void gameResultMessage_게임_결과_메시지(int strikeCount, int ballCount) {
        assertAll(
    ...
    void gameOver_게임_종료_여부(int strikeCount) {
        assertTrue(strikeCount == MAX_LENGTH, "게임 계속 진행");
    }

    @ParameterizedTest
    @DisplayName("게임 종료, 재시작 입력 숫자 검증")
    @ValueSource(ints = { 0, 1, 2, 3 })
    void gameProcessType_게임_종료_여부(int retry) {
        assertTrue(retry == GameProcessType.GAME_RETRY.getKey() || retry == GameProcessType.GAME_OVER.getKey(), "1, 2만 입력 가능");
    }
}